...
public class Mine extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        if (message[0].equalsIgnoreCase("mine")) {
            int pickaxeDurability = Integer.parseInt(db.getColumn(member.getId(), "pickaxe", "scores"));
            ...
            db.setColumn(member.getId(), "pickaxe", "" + (pickaxeDurability - 1), "scores");
    ...
    private void handleCommon(EmbedBuilder eb, Member member) {
        ...
        if (rand2 == 0) {
            ...
            int gearAmount = Integer.parseInt(db.getColumn(member.getId(), "gear", "scores"));
            db.setColumn(member.getId(), "gear", "" + (gearAmount + 1), "scores");

        } else if (rand2 == 1) {

            eb.addField("**Results**", "You found a wrench \uD83D\uDD27 Maybe try again?", false);
            int wrenchAmount = Integer.parseInt(db.getColumn(member.getId(), "wrench", "scores"));
            db.setColumn(member.getId(), "wrench", "" + (wrenchAmount + 1), "scores");

        } else if (rand2 == 2) {

            eb.addField("**Results**", "You found a rusty key \uD83D\uDD11 Doubt it'd be useful for anything...", false);
            int rustyKeyAmount = Integer.parseInt(db.getColumn(member.getId(), "rustyKey", "scores"));
            db.setColumn(member.getId(), "rustyKey", "" + (rustyKeyAmount + 1), "scores");

        } else if (rand2 == 3) {

            eb.addField("**Results**", "You found a battery \uD83D\uDD0B Must've been from some previous miner.", false);
            int batteryAmount = Integer.parseInt(db.getColumn(member.getId(), "battery", "scores"));
            db.setColumn(member.getId(), "battery", "" + (batteryAmount + 1), "scores");
    ...
    private void handleRare(EmbedBuilder eb, Member member) {
        ...
        db.setColumn(member.getId(), "exp", "" + (profile.getMemberExp(member) + expToGain), "scores");
        ...
        if (rand2 == 0) {
            ...
            int moneyBagAmount = Integer.parseInt(db.getColumn(member.getId(), "moneyBag", "scores"));
            db.setColumn(member.getId(), "moneyBag", "" + (moneyBagAmount + 1), "scores");
    ...
    private void handleExtraOrd(EmbedBuilder eb, Member member) {
        ...
        eb.addField("**Results**", "Woah, a shiny diamond?? \uD83D\uDC8E Shiny... \uD83D\uDCB8\uD83D\uDCB8\uD83D\uDCB8\n" +
                "You also gained " + (expToGain) + " experience.", false);
        int diamondAmount = Integer.parseInt(db.getColumn(member.getId(), "diamond", "scores"));
        db.setColumn(member.getId(), "diamond", "" + (diamondAmount + 1), "scores");
    }
    ...
    private void handleMythical(EmbedBuilder eb, Member member) {
        ...
        if (rand2 == 0) {
            ...
            eb.addField("**Results**", "A Mythical Knife \uD83D\uDD2A Only legends talked about these.\n" +
                    "You also gained " + (expToGain) + " experience.", false);
            db.setColumn(member.getId(), "mKnife", "" + (inventory.getMythicalKnife(member) + 1), "scores");

        } else if (rand2 == 1) {

            eb.addField("**Results**", "A Mythical Shield \uD83E\uDDFF No one will mess with you.\n" +
                    "You also gained " + (expToGain) + " experience.", false);
            db.setColumn(member.getId(), "mShield", "" + (inventory.getMythicalShield(member) + 1), "scores");


        } else if (rand2 == 2) {
            eb.addField("**Results**", "A Mythical Hourglass ⌛ This amulet let's you travel through time.\n" +
                    "You also gained " + (expToGain) + " experience.", false);
            db.setColumn(member.getId(), "hourglass", "" + (inventory.getHourglass(member) + 1), "scores");
    ...
    private void handleLegendary(MessageReceivedEvent event, EmbedBuilder eb, Member member) {
        ...
        eb.addField("**Results**", "You claimed A Legendary Amulet \uD83C\uDFC5 This will make you unstoppable in the long run.\n" +
                "You also gained " + (expToGain) + " experience.", false);
        int currAttack = Integer.parseInt(db.getColumn(member.getId(), "attack", "scores"));
        db.setColumn(member.getId(), "legAmulet", "" + (inventory.getLegendaryAmulet(member) + 1), "scores");
        db.setColumn(member.getId(), "amuletMult", "" + (inventory.getAmuletMuliplier(member) + 1), "scores");
